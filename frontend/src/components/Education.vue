...
const filteredEducation = computed(() => educationHistory.value.filter(
  e => e && e.period && e.institution
))
...
<template>
  <section id="pendidikan" class="relative py-24 bg-gradient-to-br from-gray-50 to-blue-100 overflow-hidden">
    ...
    <div class="container mx-auto px-6 z-10 relative">
      ...
      <div v-if="!loading && !error && filteredEducation.length > 0" class="relative mt-12">
        <div class="absolute left-1/2 transform -translate-x-1/2 w-1 h-full bg-blue-400 rounded-full shadow-lg"></div>

        <div class="space-y-16">
          <div v-for="(edu, index) in filteredEducation" :key="edu.id" :class="{
      ...
      <div v-else-if="!loading && !error && filteredEducation.length === 0" class="text-gray-500 text-center mt-8 text-lg">
